...
// customer/admin see.
//
// UNIQUENESS (spec section 16): each of the four bridge ids may belong
// to only one ACTIVE (non-removed) owner at a time. This function
// checks-then-inserts inside ONE transaction (the caller wraps this in
// BEGIN/COMMIT) and the DB's own partial UNIQUE index
// (idx_owned_nodes_special_bridge_active, see lib/db.js) is the final,
// unbypassable guarantee -- so even a hypothetical race can never result
// in the same bridge id being active on two accounts simultaneously; the
// losing INSERT would violate the index and throw, which the caller's
// transaction wrapper turns into a rollback.
export function assignSpecialBridge(db, accountId, bridgeId) {
  ...
  const existingActive = db
    .prepare(`SELECT id FROM owned_nodes WHERE special_bridge_id = ? AND removed_at IS NULL`)
    .get(bridgeId);
  if (existingActive) {
...
// Returns { [bridgeId]: accountId } for every special Bridge id currently
// ACTIVELY assigned (removed_at IS NULL) to any account -- used by the
// admin Add Bridge popup so the UI can show "already assigned" for any
// of the four special Bridges and prevent choosing it, per spec section
// 16 ("the admin UI should make that clear and prevent another
// assignment").
export function listSpecialBridgeAssignments(db) {
  const rows = db
    .prepare(
      `SELECT special_bridge_id, account_id FROM owned_nodes
       WHERE special_bridge_id IS NOT NULL AND removed_at IS NULL`
    )
    .all();
  const byBridgeId = {};
  for (const row of rows) {
    byBridgeId[row.special_bridge_id] = row.account_id;
  }
  return byBridgeId;
}
