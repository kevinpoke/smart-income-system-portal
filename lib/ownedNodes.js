...
import { getSpecialBridgeById, currentEffectiveMonthlyCentsForBridgeId } from "./specialBridges";
...
// Production feature/fix batch (unlimited Bridges decision): the
// MAX_NODES_PER_ACCOUNT = 5 arbitrary app-level cap has been removed
// entirely per approved decision -- there is no longer any maximum
// number of Nodes an account may own. Permissions, DB relationships
// (UNIQUE(account_id, node_number)), and node_number allocation logic
// are all unchanged; only the count>=N rejection branches below are
// gone.

// Default-provisioning policy (current phase): every AUTOMATIC Node
// grant -- i.e. every Node this app creates on its own, without an
// admin explicitly picking a tier -- is a Standard Node. This used to
// be a ~20% random Standard/Super split (with an unused `package_tier`
// "super" signal that could force Super on the first Node), but per
// spec that produced newly-created accounts that could start on a
// Super Node, which is no longer allowed. Nova was never auto-assigned
// here and still isn't.
//
// Future architecture note: once JVZoo purchase/product data is wired
// up, the purchased product will determine the initial Node tier
// (Standard purchase -> Standard Node, Super purchase -> Super Node,
// Nova purchase -> Nova Node). That mapping is NOT implemented yet --
// for now this is hardcoded/canonical to exactly 1 Standard Node.
const AUTO_GRANT_TIER_KEY = "standard";

// Deterministic (seeded, never Math.random()) rate selection for the
// AUTOMATIC first-Node grant. Returns { tierKey, tier (display name),
// estMonthlyCents }. Tier is always Standard -- see
// AUTO_GRANT_TIER_KEY above.
function deterministicAutoTierSpec(accountId, nodeNumber) {
  const tierKey = AUTO_GRANT_TIER_KEY;
...
// granted.
//
// This is the AUTOMATIC grant path -- every Node it creates is Standard
// tier (see AUTO_GRANT_TIER_KEY above); it never creates a Super or
// Nova Node. See addOwnedNodeWithTier() below for the admin-driven Add
// Node flow, which always uses an explicitly chosen tier instead.
// `created_at` is always "now" (real wall-clock time of this call), so
// a newly granted Node's earnings only ever start accruing from the
// moment it is actually created -- never retroactively.
export function addOwnedNode(db, accountId) {
  const nextNodeNumberRow = db
    .prepare(`SELECT COALESCE(MAX(node_number), 0) as m FROM owned_nodes WHERE account_id = ?`)
    .get(accountId);
  const nodeNumber = nextNodeNumberRow.m + 1;
  const spec = deterministicAutoTierSpec(accountId, nodeNumber);
...
export function addOwnedNodeWithTier(db, accountId, tierKey) {
  ...
  // Node numbering NEVER reuses a removed Node's number ("not reuse the
  // removed Node's identity," "not renumber remaining Nodes") -- the
  // next node_number is always one past the HIGHEST node_number this
  // account has ever had, active or removed, not merely a count of
...
export function updateOwnedNodeTier(db, accountId, ownedNodeId, newTierKey) {
  ...
  const node = db
    .prepare(`SELECT * FROM owned_nodes WHERE id = ? AND account_id = ? AND removed_at IS NULL`)
...
// Admin-only: soft-removes ONE specific owned Node from an account. Sets
// `removed_at` to now (never deletes the row -- see the removed_at
// column comment in lib/db.js for why). Scoped identically to
// updateOwnedNodeTier() (`WHERE id = ? AND account_id = ? AND
// removed_at IS NULL`) so this can never accidentally remove a
// DIFFERENT Node belonging to the account, and calling it twice on an
// already-removed Node is a safe, idempotent "not_found" (never a
// double-removal / never re-stamps removed_at with a newer time).
//
// Returns the full pre-removal snapshot the caller needs for its audit
// row (nodeId, displayNodeId, nodeNumber, tier, tierKey, the STABLE
// earning rate, createdAt, and totalEarnedCents at removal time) --
// totalEarnedCents is looked up by the caller via
// computeNodeEarningsTotals() BEFORE calling this function (removal
// itself never touches ledger_entries, so that total is unaffected by
// the order of operations, but the caller passes it in here purely so
// this function's return shape is self-contained for the audit_log
// write).
export function removeOwnedNode(db, accountId, ownedNodeId) {
  const node = db
    .prepare(`SELECT * FROM owned_nodes WHERE id = ? AND account_id = ? AND removed_at IS NULL`)
    .get(ownedNodeId, accountId);
  if (!node) {
  ...
  const now = new Date().toISOString();
  db.prepare(`UPDATE owned_nodes SET removed_at = ? WHERE id = ? AND account_id = ?`).run(
    now,
    ownedNodeId,
    accountId
  );
  ...
  return {
    ...
    tier: node.tier,
    tierKey: displayNameToTierKey(node.tier),
    earningRateCents: node.earning_rate_cents ?? node.est_monthly_cents,
    createdAt: node.created_at,
    removedAt: now,
  };
}

// ---- Special admin-assignable Bridges (ISP support controls + special
// bridges batch) --------------------------------------------------------
//
// Assigns ONE of the four EXACT special Bridges (spec section 13/14) to
// a customer account. Reuses the exact same owned_nodes table/columns
// every other owned Bridge already uses (never a second, parallel
// table) -- tier/est_monthly_cents/earning_rate_cents are seeded to the
// catalog's BASE value (the live +/-2-5% effective rate is computed at
// READ time by listOwnedNodes()/the earnings engine, never stored/
// mutated here, so this row stays a stable historical anchor).
// display_node_id is set to the EXACT bridge id string (e.g. "284373"),
// never a generated display id, so "#284373" is exactly what the
// customer/admin see.
//
// REUSABLE-SPECIAL-BRIDGES batch: each special Bridge id is a reusable
// TEMPLATE, not single-owner inventory -- the SAME bridge id may be
// ACTIVELY owned by any number of DIFFERENT accounts simultaneously
// (spec: "There should be NO global 'already assigned' restriction for
// special Bridges"). The only remaining uniqueness guarantee is
// PER-ACCOUNT: the same bridge id may belong to only one ACTIVE
// (non-removed) row for a given account at a time, to reject an
// accidental duplicate assignment to the SAME customer. This function
// checks-then-inserts inside ONE transaction (the caller wraps this in
// BEGIN/COMMIT) and the DB's own partial UNIQUE index
// (idx_owned_nodes_special_bridge_per_account, see lib/db.js) is the
// final, unbypassable guarantee scoped to (account_id, special_bridge_id)
// -- so even a hypothetical race can never result in the same bridge id
// being active twice on the SAME account, while two DIFFERENT accounts
// each actively owning that same bridge id is the intended, supported
// state.
export function assignSpecialBridge(db, accountId, bridgeId) {
  const bridge = getSpecialBridgeById(bridgeId);
  if (!bridge) {
    return { added: false, reason: "invalid_bridge_id" };
  }

  const existingActiveForAccount = db
    .prepare(
      `SELECT id FROM owned_nodes WHERE account_id = ? AND special_bridge_id = ? AND removed_at IS NULL`
    )
    .get(accountId, bridgeId);
  if (existingActiveForAccount) {
    return { added: false, reason: "already_assigned" };
  }
  ...
  const now = new Date().toISOString();
  const id = generateId("ownednode");
  // tierKeyToDisplayName() maps the internal tier key ("super"/"nova") to
  // the pre-existing internal owned_nodes.tier display string ("Super
  // Node"/"Nova Node") -- this is deliberately the SAME internal value
  // every non-special Bridge of that tier already stores, per spec
  // section 14 ("use the existing compatible internal key"). The
  // CUSTOMER-facing "Golden Bridge"/"IX Bridge" label is derived
  // separately at read time from special_bridge_id (see
  // lib/ownedNodes.js#listOwnedNodes and lib/specialBridges.js), never
  // from this internal tier string.
  const internalTierDisplayName = tierKeyToDisplayName(bridge.tierKey);

  db.prepare(
    `INSERT INTO owned_nodes
       (id, account_id, node_number, tier, est_monthly_cents, earning_rate_cents, created_at, display_node_id, special_bridge_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    id,
    accountId,
    nodeNumber,
    internalTierDisplayName,
    bridge.baseEstMonthlyCents,
    bridge.baseEstMonthlyCents,
    now,
    bridge.id,
    bridge.id
  ...
  return {
    ...
    baseEstMonthlyCents: bridge.baseEstMonthlyCents,
    createdAt: now,
  };
}

// REUSABLE-SPECIAL-BRIDGES batch: returns the set of special Bridge ids
// the GIVEN account currently ACTIVELY owns (removed_at IS NULL) --
// used by the admin Add Bridge popup so the UI can show "Already
// Assigned" and prevent a duplicate pick ONLY for the customer
// currently being edited. Deliberately scoped to ONE account (not a
// global bridgeId->accountId map anymore) -- special Bridges have no
// global "taken by someone else" state; a bridge already owned by a
// DIFFERENT account must still show as freely assignable here.
export function listSpecialBridgeAssignmentsForAccount(db, accountId) {
  const rows = db
    .prepare(
      `SELECT special_bridge_id FROM owned_nodes
       WHERE account_id = ? AND special_bridge_id IS NOT NULL AND removed_at IS NULL`
    )
    .all(accountId);
  return new Set(rows.map((row) => row.special_bridge_id));
}
...
export function ensureOwnedNode(db, account) {
  ...
  const countRow = db
    .prepare(`SELECT COUNT(*) as c FROM owned_nodes WHERE account_id = ? AND removed_at IS NULL`)
    .get(account.id);
  if (countRow.c > 0) {
...
export function listOwnedNodes(db, account) {
  const rows = db
    .prepare(
      `SELECT id, node_number, tier, est_monthly_cents, earning_rate_cents, created_at, display_node_id, special_bridge_id FROM owned_nodes
  ...
  return rows.map((r, index) => {
    // ISP support controls + special bridges batch: for one of the four
    // special Bridges, the customer/admin-facing display name and the
    // "current" monthly earnings figure both come from the DETERMINISTIC
    // effective-rate calculation (base estimate x this calendar month's
    // stable +/-2-5% variance -- lib/specialBridges.js), never the raw
    // stored est_monthly_cents/earning_rate_cents (which stay pinned to
    // the catalog's BASE value -- see assignSpecialBridge() below -- so
    // the stored row is a stable historical anchor, and the live display
    // always reflects the CURRENT month's effective rate on every read,
    // with zero migration needed when a new calendar month begins).
    const special = r.special_bridge_id ? getSpecialBridgeById(r.special_bridge_id) : null;
    const specialEffectiveMonthlyCents = special
      ? currentEffectiveMonthlyCentsForBridgeId(r.special_bridge_id)
      : null;
    return {
      ...
      location,
      estMonthlyCents: specialEffectiveMonthlyCents ?? r.est_monthly_cents,
      earningRateCents: r.earning_rate_cents ?? r.est_monthly_cents,
      createdAt: r.created_at,
      isPrimary: index === 0,
      // Special-Bridge-only fields; null/false for every normal Bridge.
      specialBridgeId: r.special_bridge_id || null,
      isSpecialBridge: Boolean(special),
      specialBridgeDisplayName: special?.displayName || null,
      baseEstMonthlyCents: special?.baseEstMonthlyCents ?? null,
    };
  });
}
...
export function sumOwnedNodesMonthlyCents(db, accountId) {
  const row = db
    .prepare(`SELECT COALESCE(SUM(est_monthly_cents), 0) as total FROM owned_nodes WHERE account_id = ? AND removed_at IS NULL`)
    .get(accountId);
  return row.total;
}
...
// NODE RULE comment at the top of this file. Excludes removed Nodes --
// a removed Node can never be "primary" for any purpose (Node column
// display, admin popup default selection, etc).
    ...
    db
      .prepare(
        `SELECT id, node_number, tier, est_monthly_cents, earning_rate_cents, created_at, display_node_id
         FROM owned_nodes WHERE account_id = ? AND removed_at IS NULL ORDER BY node_number ASC LIMIT 1`
      )
      .get(accountId) || null
  );
}
...
export function computeNodeEarningsTotals(db, accountId) {
  const nodes = db
    .prepare(`SELECT id, est_monthly_cents FROM owned_nodes WHERE account_id = ? AND removed_at IS NULL ORDER BY node_number ASC`)
    .all(accountId);
