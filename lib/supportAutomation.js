...
// Due-scan entry point, called from the shared background scheduler tick
// (lib/backgroundScheduler.js) -- covers BOTH existing customers already
// >= 4 days old at the moment this feature is deployed (spec: "still
// eligible -> send once after deployment/background scan picks them up")
// AND future customers who cross the 4-day threshold later, since this
// scan re-runs on every tick and re-evaluates every currently-eligible
// account fresh each time. Candidates are found via a cheap indexed
// query (role/status/waitlist/created_at, plus a NOT EXISTS against this
// event's own key so an account that already received it is never
// re-considered); the actual send-time recheck + idempotent insert still
// happens inside sendNonWaitlist4DayMessage() above for every candidate,
// so this candidate query is only ever a performance filter, never the
// real safety guard.
