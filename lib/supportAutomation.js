...
// STOP-AUTOMATED-SUPPORT-MESSAGES batch: this entire automation
// EXECUTION PATH is now permanently disabled. scheduleMessage() is no
// longer called anywhere in the app (see the removed call sites in
// app/api/auth/login/route.js and lib/ispEngine.js -- confirmed via a
// full-tree search before this change), and deliverDueMessages() below
// now unconditionally returns without ever copying a row into
// support_messages, regardless of what may still be sitting in
// scheduled_support_messages. This is a deliberate belt-and-suspenders
// pairing with the lib/db.js migration that stamps `cancelled_at` on
// every already-pending row: even if some future code path were ever
// mistakenly wired back up to call deliverDueMessages(), it could not
// resurrect delivery of old or new automated messages. Historical rows
// already copied into support_messages (delivered_at IS NOT NULL,
// before this change) are NEVER touched or deleted by any of this --
// they remain visible in the customer's and admin's Support inbox
// exactly as before. Manual customer/admin messages are posted through
// an entirely separate code path (lib/supportEngine.js#postMessage(),
// called only from the customer/admin Support POST routes) and are
// completely unaffected.
export function deliverDueMessages(_db, _accountId) {
  return { delivered: 0 };
}

// Idempotently schedules ONE message for delivery at `deliverAt` (a Date
// or ISO string). RETAINED for backward-compat/reference only -- no
// code path in this app calls this function anymore (see the comment
// on deliverDueMessages above), and even if it were called, the row it
// inserts would never be delivered since deliverDueMessages() is now a
// permanent no-op. `eventKey` is the sole idempotency guard -- if a row
