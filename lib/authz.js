...
export function toPublicAccount(account) {
  ...
  return {
    ...
    // OTHER-STATE-ISP batch: explicit Other flag + the customer's own
    // typed region text, exposed read-only so the ISP Setup page (post-
    // submission "Application Status"/active views) and any customer-
    // facing summary can reflect it. Never re-derived from ispState text.
    ispStateIsOther: Boolean(account.isp_state_is_other),
    ispStateOtherText: account.isp_state_other_text || null,
    ...
    // Refinement pass: safe PUBLIC URL only (never raw image bytes/
    // base64) -- see lib/uploads.js and app/api/profile/photo.
    ...
    // Portal reliability pass: ISP "I Approve" -> Node-activation
    // authorization in-progress marker. Mirrors wifiReconnectStartedAt
    // exactly -- non-null only while the 20-second "Establishing a
    // Secure Connection..." authorization window is underway, so a
    // refresh mid-flow can resume the correct remaining progress instead
    // of restarting or getting stuck at 0%.
    ispAuthorizeStartedAt: account.isp_authorize_started_at,
    // Production feature/fix batch: independent unread indicator for the
    // ISP Setup nav tab, driven by accounts.isp_unread -- set to 1 by
    // lib/ispEngine.js#transitionIspToApproved (manual OR automatic
    // approval) and cleared only when the customer actually loads the
    // ISP Setup page (see POST /api/isp/mark-seen). Deliberately fully
    // independent of Support's customer_unread column -- opening one
    // must never clear the other.
    ispUnread: Boolean(account.isp_unread),
