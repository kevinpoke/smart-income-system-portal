...
// JVZOO-BRIDGE-UPSELL batch: the two upsell products that grant a
// Bridge entitlement (never base account access -- an upsell BUY for
// an email with no existing Smart Income System account is never
// enough context to originate one, see the webhook route). Confirmed
// directly by the user:
//   452855 -- "Standard Bridge Oto1" -> +1 Standard Bridge
//   452859 -- "IX Bridge Oto2"       -> +1 IX Bridge (internal tier key
//             stays "nova", per lib/nodeTiers.js; never renamed)
// STANDARD-SECTION ONLY: both map to the generic pickRateForTier()
// generator via lib/nodeTiers.js -- NEVER to lib/specialBridges.js's
// SPECIAL_BRIDGE_CATALOG. Keep this Set/Map isolated in this one file
// (mirrors the APPROVED_PRODUCT_IDS isolation pattern above) so a
// future upsell can be added/changed in one place.
export const JVZOO_UPSELL_PRODUCT_IDS = new Set(["452855", "452859"]);

export const JVZOO_UPSELL_PRODUCT_TIER_KEY = {
  "452855": "standard",
  "452859": "nova",
};

export function isJvzooUpsellProductId(productId) {
  return JVZOO_UPSELL_PRODUCT_IDS.has(String(productId));
}
