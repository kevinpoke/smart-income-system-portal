...
function postMessageInner(db, { conversationId, senderRole, senderAccountId = null, body }) {
  const now = new Date().toISOString();
...
export function postMessage(db, { conversationId, senderRole, senderAccountId = null, body }) {
  ...
  db.exec("BEGIN");
  try {
    const result = postMessageInner(db, { conversationId, senderRole, senderAccountId, body });
    db.exec("COMMIT");
    return result;
  } catch (err) {
    db.exec("ROLLBACK");
