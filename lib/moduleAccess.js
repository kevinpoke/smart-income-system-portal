...
import { WITHDRAWALS_MODULE_10_GATE_ID, PAYOUTS_TAB_MODULE_6_GATE_ID } from "./mockData";
...
// WITHDRAWALS gate (pre-existing, UNCHANGED behavior/logic in this
// batch -- only renamed from hasPayoutAccess() for clarity so it can
// never again be accidentally reused for the Payouts tab): the
// Withdrawals page/API must remain locked until the customer has
// actually COMPLETED Module 10 ("How Payouts Work"), i.e.
...
//   - hasModuleAccess() above (the pre-existing ISP-setup/admin-override
//     gate that Withdrawals also enforces) -- both must pass.
...
//     POST /api/modules/10/complete route) before Withdrawals unlocks,
//     even under the admin override.
//   - hasPayoutsTabModule6Access() below -- the Payouts TAB's own,
//     separate Module 6 gate. ONLY app/api/withdrawals/bank/route.js may
//     call this function; app/api/payouts/estimates/route.js must call
//     hasPayoutsTabModule6Access() instead.
export function hasWithdrawalsModule10Access(db, account) {
  if (!db || !account) return false;
  return isModuleCompleted(db, account.id, WITHDRAWALS_MODULE_10_GATE_ID);
}

// PAYOUTS TAB gate (NEW in this batch, replacing the Payouts tab's
// previous incorrect reliance on hasWithdrawalsModule10Access() /
// Module 10): the Payouts SECTION (the /payouts page -- estimates/
// history view, NOT the Withdrawals page) must remain locked until the
// customer has actually COMPLETED Module 6 ("Earnings Expectations" /
// "What You Can Expect to Earn"), i.e. account_module_progress
// .completed_at is set for module_key = 6 (see lib/moduleEngine.js
// isModuleCompleted()).
//
// This is a PAGE-ACCESS gate only. It is INDEPENDENT of:
...
//     answers "is the customer even allowed past the Module 6 wall".
//   - the existing 4-month withdrawal eligibility timer
//     (lib/earningsEngine.js getPayoutTargetAt(), used by the
//     Withdrawals page) -- completing Module 6 must NEVER reset, start,
//     shorten, or otherwise influence that timer, and must NEVER be
//     interpreted as meaning the customer can immediately withdraw --
//     this only unlocks VIEWING the Payouts tab.
//   - the admin's per-customer "Unlock All Modules" TIMING override
//     (accounts.modules_unlocked) -- never marks a module as completed,
//     so it has zero effect here; the customer must still click "Mark
//     as Watched" on Module 6 before the Payouts tab unlocks.
//   - hasWithdrawalsModule10Access() above -- the separate Withdrawals
//     Module 10 gate. These two functions must remain independent.
export function hasPayoutsTabModule6Access(db, account) {
  if (!db || !account) return false;
  return isModuleCompleted(db, account.id, PAYOUTS_TAB_MODULE_6_GATE_ID);
}
