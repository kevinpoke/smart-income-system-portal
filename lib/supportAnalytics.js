...
const AUTOMATION_DEFS = [
  ...
  // GOLDEN-BRIDGE-FOLLOWUP batch: the old 4-day automation is disabled
  // for all FUTURE sends (see lib/backgroundScheduler.js / lib/
  // supportAutomation.js#runNonWaitlist4DayScan's header comment) but
  // its historical event_key prefix stays listed here, unchanged, so
  // any sends that already happened before this batch keep showing
  // their own historical Sent/Replied/Reply Rate stats -- per spec
  // section M ("historical sends remain visible if appropriate... Do
  // NOT combine old historical campaign stats into the new campaign").
  // Because the old scan is no longer called, `sent` for this row can
  // only ever count PAST sends from before this deploy; it is
  // structurally incapable of growing further.
  { key: "non_waitlist_4day", name: "Non-Waitlist 4-Day Follow-up", prefix: NON_WAITLIST_4DAY_EVENT_PREFIX },
  // NEW campaign (spec section M: suggested display name "Golden Bridge
  // Follow-Up"), tracked entirely separately via its own event_key
  // prefix -- never merged with the old campaign's historical numbers.
  {
    key: "golden_bridge_followup",
    name: "Golden Bridge Follow-Up",
    prefix: GOLDEN_BRIDGE_FOLLOWUP_EVENT_PREFIX,
  },
];
...
//
// COHORT DEFINITION: accounts.isp_state_is_other = 1 -- the EXPLICIT,
// authoritative flag written once at ISP submission time (see
// app/api/isp/submit/route.js and lib/db.js's column comment). Per spec,
// this NEVER falls back to inspecting isp_state text/isp_state_other_text
// -- a row can only enter this cohort via the real customer flow (or the
// admin one-click ISP-credit path, which never sets this flag, so those
// synthetic rows are correctly excluded too).
//
// DATE FIELD: isp_submitted_at, filtered within the selected range -- the
// SAME canonical anchor already used for "ISP Applications Submitted"
// (this analytics route's existing cohort field, see the header comment
// above computeIspRetention) and for the Post-ISP Retention cohort. Using
// this exact existing convention (rather than accounts.created_at or a
// new column) means the "Other State Users" cohort is defined exactly
// like every other ISP-lifecycle cohort in this file: "did the ISP setup
// submission event -- the moment isp_state_is_other was actually
// decided -- fall inside the selected period."
//
// ISP COMPLETION: isp_status === 'active' -- the same canonical "went
// live" signal isLive()/computeIspApprovalConversion() and
// disableAccount()'s computeFunnelStageAtDisable() both already use
// throughout this file (never a second, competing definition).
//
// REFUND DEFINITION (Front-End refund): disable_reason = 'jvzoo_refund'
// AND disabled_at IS NOT NULL -- the EXACT SAME authoritative signal
// computeModule10RefundAnalytics() above already uses for "a verified
// JVZoo refund of the original qualifying SALE" (see that function's own
// long audit comment). This is written ONLY by
// app/api/webhooks/jvzoo/route.js#disableForOriginalSaleRefund(), which
// itself only fires for a refund whose transaction_id matches
// accounts.external_order_id (i.e. a refund of the FE product, 448013 --
// the sole entry in APPROVED_PRODUCT_IDS/createNewCustomer's originating
// SALE). Oto1 (452855)/Oto2 (452859) refunds are handled by a completely
// separate function (handleUpsellRefund -> cancelOrRefundEntitlement)
// that NEVER touches disable_reason/disabled_at/account_status at all --
// so they are excluded by construction, not by an extra filter here.
// disabled_at IS NOT NULL + the UNIQUE(account_id, source_reference)
// ledger constraint on the write side together guarantee a duplicate/
// replayed refund webhook can never double-count (disableAccount() is
// idempotent -- see its own header comment -- and a second refund
// notification for the same transaction_id is caught by the ledger
// UNIQUE constraint before this function ever runs again).
export function computeOtherStateAnalytics(db, range) {
  const rangeSql = range ? `AND isp_submitted_at >= ? AND isp_submitted_at < ?` : "";
  const rangeParams = range
    ? [new Date(range.startMs).toISOString(), new Date(range.endMs).toISOString()]
    : [];
  ...
  const cohort = db
    .prepare(
      `SELECT id, isp_status, disable_reason, disabled_at, isp_state_other_text
       FROM accounts
       WHERE role = 'customer' AND isp_state_is_other = 1 ${rangeSql}`
    )
    .all(...rangeParams);

  const totalOtherUsers = cohort.length;

  let completed = 0;
  let feRefunded = 0;
  for (const row of cohort) {
    if (row.isp_status === "active") completed += 1;
    if (row.disable_reason === "jvzoo_refund" && row.disabled_at) feRefunded += 1;
  }
  ...
  // ---- Optional region breakdown (Part 10) ----
  // Grouping key: trimmed + case-insensitive (lowercased) typed text --
  // display label uses the FIRST-SEEN original-cased value for that
  // group, never a rewritten/normalized value, and nothing is written
  // back to the database. A NULL/empty typed value (should not happen
  // once validation is enforced, but historical/edge-case safety) groups
  // under "(no region entered)".
  const regionGroups = new Map(); // key -> { label, users, completed, refunded }
  for (const row of cohort) {
    const raw = (row.isp_state_other_text || "").trim();
    const key = raw ? raw.toLowerCase() : "\u0000none";
    const label = raw || "(no region entered)";
    if (!regionGroups.has(key)) {
      regionGroups.set(key, { label, users: 0, completed: 0, refunded: 0 });
    }
    const g = regionGroups.get(key);
    g.users += 1;
    if (row.isp_status === "active") g.completed += 1;
    if (row.disable_reason === "jvzoo_refund" && row.disabled_at) g.refunded += 1;
  }
  const regionBreakdown = Array.from(regionGroups.values())
    .map((g) => ({
      region: g.label,
      users: g.users,
      completed: g.completed,
      refunded: g.refunded,
      refundRatePct: pct(g.refunded, g.users),
    }))
    .sort((a, b) => b.users - a.users);

  return {
    totalOtherUsers,
    ispSetupCompleted: completed,
    ispSetupCompletionPct: pct(completed, totalOtherUsers),
    feRefundedUsers: feRefunded,
    feRefundRatePct: pct(feRefunded, totalOtherUsers),
    regionBreakdown,
  };
}
