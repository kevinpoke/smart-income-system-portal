...
// catch-up without double counting" rule. Each cycle's total amount is
...
//
// Dashboard adjustment pass: each cycle's total final amount is now split
// PROPORTIONALLY across the account's owned Nodes (by monthly-earnings
// share, via lib/ownedNodes.js splitProportionally) at WRITE time, with
// one ledger row per Node carrying that Node's own node_id and a
// per-Node-unique source_reference (so the existing
// UNIQUE(account_id, source_reference) idempotency guarantee still holds
// row-by-row). This is what lets the Dashboard "Your Nodes" table show
// each Node's own exact cumulative contribution instead of dividing the
// account balance equally after the fact.
//
...
export function runEarningsCatchup(db, account, totalMonthlyCents) {
  ...
  while (cursorMs < currentCycleStartMs && iterations < maxIterations) {
    ...
    // drift. cycleDurationMs is used (instead of the constant DAY_MS)
    // for the onFraction calculation below so a customer who was
    // connected for the ENTIRE 23h or 25h transition day is credited
    // exactly one full day's base amount -- never more, never less --
    // matching every ordinary 24h day's semantics.
    const cycleEndMs = cycleEndMsFor(cursorMs);
    const cycleDurationMs = cycleEndMs - cursorMs;
    const onMs = computeOnMsInRange(db, account.id, connectedAtMs, cursorMs, cycleEndMs);
    const onFraction = onMs / cycleDurationMs;
    ...
    // Split this cycle's total final amount across the account's owned
    // Nodes by monthly-earnings share, and write one ledger row per
    // Node -- each row's own final_amount_cents IS that Node's exact
    // contribution for this cycle (not a display-time estimate).
    const perNodeSplit = splitProportionally(finalCents, nodes);
    const perNodeBaseSplit = splitProportionally(baseCents, nodes);

    for (const node of nodes) {
      const sourceRef = `earning-cycle:${cycleKey}:node:${node.id}`;
      const nodeFinalCents = perNodeSplit[node.id] ?? 0;
      const nodeBaseCents = perNodeBaseSplit[node.id] ?? 0;
...
export function computeEarningsSummary(db, accountId) {
  ...
  // Divides by the CYCLE's real duration (cycleEndMs - cycleStartMs --
  // 23h/24h/25h depending on whether today is a DST transition day),
  // never the constant DAY_MS, so pacing on the two transition days per
  // year stays exactly proportional to elapsed real time instead of
  // drifting by an hour's worth of fraction.
  ...
  if (active && connectedAtMs) {
    ...
    if (accrualEnd > accrualStart) {
      ...
      todayAccruedCents = Math.round(todaysExpectedCents * (onMsToday / cycleDurationMs));
    }
  }
  ...
  // lib/ownedNodes.js computeNodeEarningsTotals) PLUS this Node's share
  // of the CURRENT in-progress cycle's live accrued amount (todayAccruedCents,
  // split the exact same proportional way as completed cycles via
  // splitProportionally), so the live number shown per-Node updates in
  // lockstep with the account-level Live Earnings ticker while WiFi is on
  // and freezes with it while WiFi is off/reconnecting. The live portion
  // is never written to the ledger -- it's derived fresh on every call,
  // exactly like the account-level todayAccruedCents it's split from.
  const completedNodeTotals = computeNodeEarningsTotals(db, accountId);
  const ownedNodeRows = db
    .prepare(`SELECT id, est_monthly_cents FROM owned_nodes WHERE account_id = ? ORDER BY node_number ASC`)
    .all(accountId);
  const liveNodeSplit =
    active && todayAccruedCents > 0 ? splitProportionally(todayAccruedCents, ownedNodeRows) : {};
  const nodeEarnings = {};
  for (const node of ownedNodeRows) {
    nodeEarnings[node.id] = (completedNodeTotals[node.id] || 0) + (liveNodeSplit[node.id] || 0);
  }
