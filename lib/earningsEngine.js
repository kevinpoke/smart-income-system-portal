...
import { clampCentsToTierMax } from "./nodeTiers";
import { getSpecialBridgeById, effectiveMonthlyCentsForBridgeMonth, monthKeyFor } from "./specialBridges";
...
export function computeNodeCycleCents({
  ...
  //
  // XI-Bridge-cap follow-up: `node.est_monthly_cents` is clamped to the
  // Node's OWN tier's current maxCents (lib/nodeTiers.js) before any
  // math is done with it. This is a pure safety net for rows persisted
  // under a since-tightened range (e.g. legacy nova/XI rows saved back
  // when the max was $2,000, before it dropped to $1,700) -- for every
  // Node whose stored rate already respects its tier's current range
  // (the overwhelming majority), Math.min() is a no-op and nothing
  // changes. See lib/nodeTiers.js#clampCentsToTierMax for the full
  // rationale and scripts/normalize-xi-cap.mjs for the companion
  // one-time DB backfill that keeps `owned_nodes.est_monthly_cents`
  // itself in sync so this clamp stays a no-op going forward.
  //
  // ISP support controls + special bridges batch: if this Node is one of
  // the four EXACT special Bridges (node.special_bridge_id set), its
  // monthly baseline for THIS SPECIFIC CYCLE is instead the
  // DETERMINISTIC, stable +/-2-5% effective rate for the CALENDAR MONTH
  // this cycle's own cycleStartMs falls in (lib/specialBridges.js) --
  // never the tier-range clamp above (a special Bridge's rate is a fixed
  // catalog base value +/- a bounded variance, not a randomly-rolled
  // tier-range rate) and never today's/now's month for a historical
  // cycle being caught up later (spec section 21: a cycle that already
  // happened in a prior month must keep using THAT month's effective
  // rate forever, never be silently repriced to the CURRENT month's rate
  // on a later catch-up run). Every other (non-special) Node's math is
  // completely unchanged.
  const specialBridge = node.special_bridge_id ? getSpecialBridgeById(node.special_bridge_id) : null;
  const cappedMonthlyCents = specialBridge
    ? effectiveMonthlyCentsForBridgeMonth(specialBridge, monthKeyFor(cycleStartMs))
    : clampCentsToTierMax(node.tier, node.est_monthly_cents);
  const nodeDailyCents = Math.round((cappedMonthlyCents / 30) * fluctuationMultiplier);
...
export function runEarningsCatchup(db, account) {
  ...
  const allNodes = db
    .prepare(
      `SELECT id, tier, est_monthly_cents, created_at, removed_at, special_bridge_id FROM owned_nodes
...
export function computeEarningsSummary(db, accountId) {
  ...
  for (let i = seriesDays - 1; i >= 0; i--) {
    ...
    // Multi-Bridge dashboard graph fix: SUM every ledger row for this
    // account+day, not just one. The prior `.get(...)` here silently
    // returned only a SINGLE arbitrary ledger_entries row (SQLite's
    // better-sqlite3-style .get() returns the first matching row) even
    // though a day with multiple owned Bridges/Nodes writes ONE
    // ledger_entries row PER NODE PER CYCLE (see runEarningsCatchup()
    // above -- the `for (const node of allNodes)` loop inserts a
    // separate row per node, each with its own node_id and its own
    // independently-computed finalCents). For a single-Bridge account
    // this bug was invisible (one row = the correct total); for a
    // multi-Bridge account the graph showed only whichever one row the
    // query happened to return, not the account's real total. Using
    // SUM(...) here reconciles the graph with every other Dashboard
    // total (todayAccruedCents, lifetimeEarningsCents, weekRow/monthRow
    // above), which have always correctly summed across every node.
    const row = db
      .prepare(
        `SELECT COUNT(*) as cnt, COALESCE(SUM(final_amount_cents), 0) as total FROM ledger_entries
         WHERE account_id = ? AND event_type = 'earning' AND effective_date = ?`
      )
      .get(accountId, ds);
    let cents = 0;
    if (row && row.cnt > 0) {
      ...
      // Today's cycle has not yet been written to the ledger (it is
      // still in progress -- see runEarningsCatchup()'s "current cycle
      // is deliberately never written" comment above) -- todayAccruedCents
      // is ALREADY the correct account-wide total across every eligible
      // owned Node for the live in-progress cycle (see the
      // `for (const node of liveEligibleNodeRows)` loop above this
      // function that builds it), so today's graph point reconciles
      // exactly with the rest of the Dashboard's "today" figures with
      // zero separate/duplicate formula.
