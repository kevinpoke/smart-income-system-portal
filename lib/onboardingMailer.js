...
// plaintext whenever SendGrid isn't configured -- acceptable for the
...
//   - If SendGrid (SENDGRID_API_KEY + EMAIL_FROM) is configured, send the
//     real email directly via the SendGrid API. The plaintext password
...
export async function sendWelcomeEmail({ to, tempPassword }) {
  ...
  if (!apiKey || !from) {
    // No live SendGrid configured: per Phase 7 spec, do NOT persist the
    // secret anywhere as a substitute. Record a redacted failure entry so
    // admins can see onboarding is blocked on SendGrid setup, and return
    ...
    recordOutbox("blocked-no-sendgrid");
    console.log(`[JVZoo onboarding email BLOCKED] to=${to} reason="SendGrid not configured"`);
    return { delivered: false, reason: "SendGrid not configured — account created, email not sent." };
  }
  ...
  try {
    const res = await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        ...
        from: { email: from, name: "Smart Income System" },
        subject: SUBJECT,
        content: [
    ...
    if (!res.ok) {
      ...
      recordOutbox("failed-sendgrid");
      console.log(`[JVZoo onboarding email FAILED] to=${to} status=${res.status}`);
      return { delivered: false, reason: `SendGrid error ${res.status}: ${errBody.slice(0, 300)}` };
    }

    recordOutbox("sendgrid");
