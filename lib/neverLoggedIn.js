...
// ---- NEVER-LOGGED-IN-EXPORT batch --------------------------------------
//
// Returns EVERY row in the permanent admin_never_logged_in_3day cohort
// (spec section A: "The export must contain the ENTIRE permanent Never
// Logged In cohort ... It must NOT export only ... current page ...
// whatever is currently loaded in React"). Deliberately has NO
// LIMIT/OFFSET at all -- this is a fundamentally different query shape
// from listNeverLoggedIn3Day() above (which is pagination-aware for the
// admin UI table), not that function called in a loop or with a huge
// page size, so the "export everything regardless of current
// pagination" guarantee is structural, not a page-size coincidence.
//
// `search`, when provided, is honored (so an "Export Filtered Results"
// affordance could reuse this same function) but the caller decides
// which button maps to which behavior -- see
// app/api/admin/never-logged-in/export/route.js, where the ONLY
// currently-wired button ("Export All") always calls this with no
// search term, per spec's preferred-implementation note ("Export All
// always exports the entire permanent cohort").
export function listAllNeverLoggedIn3Day(db, { search = "" } = {}) {
  const whereClauses = [];
  const params = [];
  const trimmedSearch = search.trim().toLowerCase();
  if (trimmedSearch) {
  ...
  return rows.map((r) => ({
    ...
    lastName: r.last_name,
    name: r.name,
    currentFirstLoginAt: r.current_first_login_at,
    accountStatus: r.account_status,
  }));
}
