...
// ISP-auto-approval/6-hour-cadence batch: the per-module hour offsets were
// changed from 16-hour increments to 6-hour increments (first_login_at
// remains the exact same fixed anchor -- ONLY the offsets changed):
//   Modules 1-3: unlocked immediately (0h)
//   Module 4:    first_login_at + 6h
//   Module 5:    first_login_at + 12h
//   Module 6:    first_login_at + 18h
//   Module 7:    first_login_at + 24h
//   Module 8:    first_login_at + 30h
//   Module 9:    first_login_at + 36h
//   Module 10:   first_login_at + 42h (FINAL module -- Remove-Module-11
//                batch: Module 11, previously +48h, has been completely
//                removed; see lib/mockData.js MODULES_META/
//                MODULE_UNLOCK_HOURS, which no longer have an id/key of
//                11 at all)
...
// watched/marked complete. Used by lib/moduleAccess.js
// hasWithdrawalsModule10Access() (gates Withdrawals behind real Module
// 10 completion) and hasPayoutsTabModule6Access() (gates the Payouts
// TAB behind real Module 6 completion) -- two independent gates, each
// reading this same shared completion-storage function for its own
// module_key.
...
// ---- MODULE-10-SUPPORT-STATUS batch --------------------------------------
//
// Shared, canonical "Mod 10" status classifier used by the admin User
// Management table's new "Mod 10" column (Support/User Management batch).
// Reuses the SAME unlock-timestamp formula (computeModuleUnlockAtMs above)
// and the SAME admin "Unlock All Modules" override semantics already used
// by every other module-access consumer in this codebase (see this file's
// computeModuleStatuses() and lib/moduleAccess.js) -- this is
// deliberately NOT a second, independently-invented rule.
//
// Priority (per spec Part 9/10), evaluated in this exact order:
//   1. WATCHED    -- account_module_progress.completed_at is set for
//      module_key = 10 (real completion; see isModuleCompleted() above).
//      This is checked FIRST and wins regardless of unlock state, because
//      completion can only ever happen after some form of access existed.
//   2. UNLOCKED   -- Module 10 is ACTUALLY accessible RIGHT NOW, i.e.
//      EITHER the admin "Unlock All Modules" override
//      (accounts.modules_unlocked) is set OR the real schedule timestamp
//      (first_login_at + MODULE_UNLOCK_HOURS[10]) has already passed as
//      of `nowMs`. This mirrors computeModuleStatuses()'s own
//      `unlocked = adminOverride || nowMs >= unlockAtMs` logic exactly --
//      Unlock All CAN produce "Unlocked" here (by design, per spec Part
//      10: "if admin Unlock All causes Module 10 to actually be
//      accessible now, then status may show Unlocked even if their normal
//      +112h timer has not elapsed"), but can NEVER by itself produce
//      "Watched" (only real completed_at can).
//   3. NOT_UNLOCKED -- neither of the above.
//
// Returns one of the exact string constants below; the API layer maps
// these to the exact display text required by spec Part 12
// ("Not Unlocked" | "Unlocked" | "Watched").
export const MODULE_10_SUPPORT_STATUS = {
  NOT_UNLOCKED: "not_unlocked",
  UNLOCKED: "unlocked",
  WATCHED: "watched",
};

export function computeModule10SupportStatus(account, moduleKey, nowMs = Date.now()) {
  const completed = Boolean(account.module10CompletedAt ?? account.completedAt);
  if (completed) return MODULE_10_SUPPORT_STATUS.WATCHED;

  const adminOverride = Boolean(account.modules_unlocked ?? account.modulesUnlocked);
  if (adminOverride) return MODULE_10_SUPPORT_STATUS.UNLOCKED;

  const unlockAtMs = computeModuleUnlockAtMs(account, moduleKey);
  if (unlockAtMs != null && nowMs >= unlockAtMs) {
    return MODULE_10_SUPPORT_STATUS.UNLOCKED;
  }

  return MODULE_10_SUPPORT_STATUS.NOT_UNLOCKED;
}
