...
export function normalizeModuleVideo(rawUrl, declaredType) {
  ...
  // D. VTurb -- the primary/sole video host for Smart Income System
  // training modules going forward (see lib/mockData.js MODULES_META,
  // where every module now declares videoType: "vturb"). Unlike the
  // Drive/YouTube/Vimeo hostname allowlists below (which exist because
  // those URL shapes could plausibly be pasted from anywhere), a VTurb
  // URL only ever reaches this function via the admin-edited,
  // developer-trusted MODULES_META config file -- never from
  // unauthenticated user input -- so any https:// URL declared with
  // videoType "vturb" is accepted and rendered via iframe only (never
  // innerHTML/raw script injection). If VTurb's actual player URLs turn
  // out to need ID extraction/rewriting (as Drive/YouTube/Vimeo do
  // below), add that here without touching any other file.
  if (declaredType === "vturb") {
