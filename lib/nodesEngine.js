...
// Waitlist redesign batch (spec section I): the customer-visible Bridge
// list must always show a Golden Bridge (super tier) as the SIXTH
// displayed row, deterministically, for every account -- not merely by
// chance of the random super/nova draw below. Row index is 0-based, so
// "6th displayed" is index 5. This is enforced by seeding the
// super-tier index set with this fixed index BEFORE the random draw
// fills in the remaining SUPER_NODE_COUNT-1 slots, so item 6 is always
// Golden while items 1-5 and 7+ keep the existing random mix/order.
const GOLDEN_ROW_INDEX = 5;
...
function tierIndices(accountId) {
  ...
  const superIndices = new Set([GOLDEN_ROW_INDEX]);
