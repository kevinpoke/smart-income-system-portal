...
// Phase 5 correction: Super Nodes specifically must use a 4x-6x
// cost-to-earnings ratio (e.g. $700 cost -> $2,800-$4,200 earnings),
// distinct from Standard Nodes' existing 3.5x-5x range.
const SUPER_EARNINGS_MULTIPLIER_MIN = 4.0;
const SUPER_EARNINGS_MULTIPLIER_MAX = 6.0;
...
// Est. Monthly Earnings is mathematically DERIVED from cost, never
// generated independently, so the ratio invariant holds by construction --
// no separate verification step could ever disagree with the generator
// itself. Super Nodes use a 4x-6x multiplier range; Standard Nodes keep
// the existing 3.5x-5x range. computeNodes() below still asserts the
// invariant defensively (tier-aware) to catch any future edit that breaks
// this relationship.
function nodeEstMonthlyCents(accountId, rowIndex, costCents, isSuper) {
  const rand = rngFromKey(`node:earn:${accountId}:${rowIndex}`);
  const [multMin, multMax] = isSuper
    ? [SUPER_EARNINGS_MULTIPLIER_MIN, SUPER_EARNINGS_MULTIPLIER_MAX]
    : [EARNINGS_MULTIPLIER_MIN, EARNINGS_MULTIPLIER_MAX];
  const multiplier = randomFloat(rand, multMin, multMax, 4);
...
export function computeNodes(accountId, customerLocation) {
  ...
  for (let i = 0; i < NODE_COUNT; i++) {
    ...
    const estMonthlyCents = nodeEstMonthlyCents(accountId, i, costCents, isSuper);

    const ratio = estMonthlyCents / costCents;
    const [multMin, multMax] = isSuper
      ? [SUPER_EARNINGS_MULTIPLIER_MIN, SUPER_EARNINGS_MULTIPLIER_MAX]
      : [EARNINGS_MULTIPLIER_MIN, EARNINGS_MULTIPLIER_MAX];
    if (ratio < multMin - 1e-9 || ratio > multMax + 1e-9) {
      // Should be unreachable given nodeEstMonthlyCents() derives directly
      // from costCents within the allowed (tier-specific) range -- guards
      // against future regressions rather than a real runtime condition.
      throw new Error(`Node earnings ratio out of bounds for row ${i} (isSuper=${isSuper}): ${ratio}`);
    }
